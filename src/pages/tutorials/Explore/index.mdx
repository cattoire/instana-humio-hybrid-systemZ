...
**Click on the Linux server host** in the picture, be sure to click on the outer containing box, not on one of the discovered servicses. You now see some details of the server.
...
You can now also drill down in the services which were discoverd by the agent on this machine. **Click on one of the ACE Integration servers** which you see in the interfaces list on the machine overview.

![](images/ace-interface-select.png)

You now get some details on this service running on the machine. You see for example that it is a JVM, you can analyze the memory consumption and if you scroll down the message flows it is running.
