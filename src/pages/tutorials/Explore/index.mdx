...
You can now also drill down in the objects which were discovered by the agent on this machine. **Click on one of the ACE Integration servers** which you see in the interfaces list on the machine overview. Best one to look at ist the EG2 instance as it is the most used one in this sample setup.
